...
if (Path.exists(hdfile)):
    ...
    rdate = pd.read_hdf(hdfile, key='lastdate')  # 读取库中记录的最后统计时间
...
if len(limit_cal)>0:
    rdate=limit_cal.iloc[-1]  #记录最后的统计日期
    ...
    pd.Series({'trade_date': rdate}).to_hdf(hdfile, 'lastdate')
    print('记录最后统计日期ok')
else :
    print('已经是最新数据，无需更新')